...
Specifically, we will use the microcanonical variant of the SBM, proposed by Peixoto \cite{Peixoto-Bayesian-Microcanonical}. To allow for degree-variability between members of the same block, we adopt the degree-corrected formulation (DC-SBM), defined in~(\ref{defn:microcan-dc-sbm}).
...
imposed by $e$, $b$ and $k$.
Specifically, if $A \sim \mbox{\rm DC-SBM}_{\rm MC} (b, e, k)$ then it must satisfy the following, for all $r,s\in[B]$
and all $i\in[N]$:
	%
	\begin{equation}
