...
which may vary with the sample index (see Appendix \ref{appdx:step-size} for a discussion of step-size variation).
...
\end{equation}
%
The multiplier $k$ in~(\ref{eqn:fij}) determines the degree of significance of the result. However, as the Laplace approximation is not exact, we only treat this dimensionality reduction method as a useful heuristic and not an exact method. Appendix~\ref{appdx:hyp-test} explores this subject at greater depth.
