...
%
\begin{equation}
	U(\theta) = \left( \sum_{i=N}^{T} \sum_{j=1}^{B} y_{ij} \log \frac{1}{a_{ij}} \right)
...
\end{equation}
%
This fully specifies, the sampling procedure to generate $\theta^{(t)} \sim p(\theta | X, b^{(t)})$. This is a pseudo-marginal approach, meaning that $\theta^{(t)} \sim p(\theta | X, A)$ as $b^{(t)} \sim p(b | A, X)$.
