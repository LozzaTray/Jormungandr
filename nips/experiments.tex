...
Next, in order to assess the performance of the feature-to-block predictor, 
...
\end{equation}
%
and where $\star \in \{0, 1\}$ indicates whehter the training or test
...
$D$ to a desired $D'$ with $k=1$ based on the maximal cutoff $c^*$. 
...
of the graphs in Figure~\ref{fig:graphs-all}. The polbooks graph yields the cleanest separation between blocks but nonetheless the inferred partitions for the other datasets do succeed at partitioning the graph into densely connected clusters.
