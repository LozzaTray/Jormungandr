...
%opening
\title{Automatic feature classification for the stochastic block model}
\author{Lawrence Tray \and I. Kontoyiannis}
...
\addbibresource{final-sources.bib}
...
\begin{figure}[!h]
	...
	% arrows
	\draw[->] (X.east) -- node[above] {$\theta = \{w_j\}_j$} (b.west);
	\draw[->] (b.east) -- node[above] {$\psi = \{e, k\}$}(A.west);
	\end{tikzpicture}
\end{figure}
